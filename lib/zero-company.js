...
export const zeroCompanyGame = {
  ...
  description:
    "Source-checked STAR WARS Zero Company issue updates, patch status, enemy-turn soft locks, crashes and stuttering, SteamOS support, manual saves and Beskar Mode."
};
...
export const zeroCompanySources = {
  ...
  issueUpdates: {
    ...
    note:
      "A pinned EA Star Wars developer post, last edited September 1, acknowledges crashes, CPU-threading work and the 100% hit-chance bug and gives current NVIDIA and Intel crash guidance."
  },
  troubleshooting: {
...
const rawZeroCompanyPages = [
  {
    slug: "issue-updates-patch-notes",
    title: "STAR WARS Zero Company Issue Updates & Patch Status",
    seoTitle: "Zero Company Issue Updates: Patch Status & Known Bugs",
    breadcrumbLabel: "Issue updates",
    cardTitle: "What is the current patch and known-issue status?",
    cardSummary:
      "Track the bugs EA has actually acknowledged, the current official workarounds and what still has no published patch, version or release date.",
    label: "ISSUE TRACKER",
    description:
      "Current STAR WARS Zero Company issue updates and patch status: crashes, CPU threading, NVIDIA and Intel guidance, the 100% hit-chance bug and unconfirmed fixes.",
    intro:
      "EA Star Wars has a pinned developer issue post for the launch build. Its latest edit acknowledges crashes, CPU-threading work, certain NVIDIA and Intel crash cases and a bug that can reduce a displayed 100% hit chance when the shot is taken. The post does not yet publish a patch version, release date or fixed-issue list, so this tracker keeps acknowledged bugs separate from player reports and patch speculation.",
    quickAnswer:
      "As checked September 3, EA says it is working on crash and CPU-threading fixes and is investigating the 100% hit-chance recalculation bug. For NVIDIA crashes while using DLSS with Game Ready Driver 610.88 or older, EA says updating the driver might help. For some Intel 13th/14th Gen desktop crashes, EA points to current BIOS guidance. No patch number, release time or official fixed list appears in the pinned issue post yet, so install only released updates and do not treat a rumored patch date as a shipped fix.",
    evidenceTitle: "Developer-acknowledged issues; no published patch version yet",
    evidenceNote:
      "The official Steam discussion is authored by EA Star Wars, carries the developer badge and was last edited September 1. It confirms the items below but does not say every crash shares one cause or that any issue is already fixed.",
    publishedAtIso: "2026-09-03",
    updatedAtIso: "2026-09-03",
    sections: [
      {
        heading: "Official STAR WARS Zero Company issue status",
        type: "table",
        columns: ["Issue", "Official status", "What you can do now"],
        rows: [
          ["Crashes and CPU threading", "EA says fixes are being implemented; no patch version or ETA is listed", "Install released updates and use the official troubleshooting order"],
          ["NVIDIA crash while using DLSS", "Game Ready Driver 610.88 or older may be involved on some PCs", "Update to the latest stable Game Ready Driver; EA says this might help"],
          ["Intel 13th/14th Gen desktop crash", "Processor instability may cause crashes on some systems", "Follow the latest BIOS guidance from the system or motherboard manufacturer"],
          ["Displayed 100% hit chance becomes lower", "EA acknowledges the chance can be recalculated when the shot is taken and is looking into a fix", "Treat the displayed 100% as unreliable until an official fix is published"],
          ["Enemy turn never ends", "Players and an independent review establish the symptom; EA's issue post does not name it separately", "Use the safe recovery guide and report repeatable mission details"]
        ],
        note:
          "Status checked September 3, 2026. Acknowledged means EA has named the issue; it does not mean a patch has shipped."
      },
      {
        heading: "Patch status: what is published and what is not",
        type: "table",
        columns: ["Question", "Current answer", "Evidence"],
        rows: [
          ["Has EA published a patch version?", "Not in the pinned issue post", "The post describes work in progress without a build number"],
          ["Is there an official release date?", "Not in the pinned issue post", "A community post or screenshot is not a released patch note"],
          ["Are crashes fixed?", "No official fixed list is published there", "EA still describes crash and CPU-threading fixes as being implemented"],
          ["Is the 100% hit bug fixed?", "No", "EA says it is looking into a fix"],
          ["Will this page change after a patch?", "Yes", "The status will move only when an official source publishes the version and fixes"]
        ]
      },
      {
        heading: "Use this safe order while waiting for a patch",
        type: "steps",
        items: [
          "Check for a released game update in Steam, the EA app, Epic, PlayStation 5 or Xbox Series X|S, then restart the device.",
          "On PC, verify or repair the game files before changing hardware or security settings.",
          "Close unnecessary background applications and update the graphics driver from the GPU manufacturer.",
          "If the crash matches the NVIDIA DLSS and old-driver case, update beyond Game Ready Driver 610.88 using NVIDIA's official path.",
          "If you use an affected Intel 13th/14th Gen desktop processor, follow the current system or motherboard manufacturer's BIOS guidance. Do not change firmware or CPU tuning you do not understand.",
          "If the issue repeats, report the platform, hardware, mission, exact error and reproduction steps to EA with a screenshot or clip."
        ]
      },
      {
        heading: "Do not confuse these evidence levels",
        type: "cards",
        items: [
          { title: "Acknowledged", text: "EA has named the issue. This is the correct label for crashes, CPU threading and the hit-chance bug." },
          { title: "Workaround", text: "A step may help a matching system. It is not a universal fix and does not prove the root cause." },
          { title: "Announced patch", text: "A future patch claim needs an official source and date. It still is not installed code." },
          { title: "Released fix", text: "Only a shipped build plus official notes or a verified retest should move an issue into the fixed column." }
        ]
      }
    ],
    faqs: [
      {
        q: "What issues has EA acknowledged in STAR WARS Zero Company?",
        a: "EA has acknowledged crashes, CPU-threading work, some NVIDIA and Intel crash cases, and a bug that can recalculate a displayed 100% hit chance to a lower value when the shot is taken."
      },
      {
        q: "Has STAR WARS Zero Company received a crash patch?",
        a: "The pinned EA issue post does not list a released patch version or fixed-issue list as of September 3, 2026. It says fixes are being implemented."
      },
      {
        q: "When is the next Zero Company patch?",
        a: "No release date appears in the pinned official issue post as of September 3. Do not treat a community post or screenshot as a shipped update; check the official source and your platform's download history."
      },
      {
        q: "Why did a 100% shot miss in Zero Company?",
        a: "EA says a known issue can recalculate a displayed 100% chance to a lower value when the shot is taken and is looking into a fix."
      },
      {
        q: "What should I try for Zero Company crashes?",
        a: "Install released updates, restart, verify or repair files, close background apps and update graphics drivers. Use the NVIDIA or Intel-specific guidance only when your hardware matches the official case."
      }
    ],
    relatedSlugs: ["crash-stutter-fix", "enemy-turn-stuck", "steam-deck"],
    sources: [
  ...
  },
  {
    ...
    relatedSlugs: ["issue-updates-patch-notes", "crash-stutter-fix", "difficulty-permadeath", "steam-deck"],
    sources: [
      zeroCompanySources.eaForumReports,
  ...
  },
  {
    ...
    relatedSlugs: ["issue-updates-patch-notes", "enemy-turn-stuck", "steam-deck", "difficulty-permadeath"],
    sources: [
      zeroCompanySources.issueUpdates,
      zeroCompanySources.troubleshooting,
      zeroCompanySources.launchFaq,
